function Banner(id, url, interval) {
    ...
    this.parent=this.banner.parentNode;
...
Banner.prototype = {
    ...
    bindData: function () {
        if (this.data) {
            ...
            for (var i = 0; i < this.data.length; i++) {
                ...
                str += '<div><img src="" realSrc="' + curData.src + '" bg="'+curData.bg+'"/></div>';
    ...
    setBannerImg : function () {
        for (var i = 0; i < this.imgs.length; i++) {
            if (i == this.step) {
                utils.css(this.imgs[i].parentNode, 'zIndex', 1);
                utils.css(this.imgs[i].parentNode.parentNode.parentNode.parentNode, 'background',this.imgs[i].getAttribute('bg'));
